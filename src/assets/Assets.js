import img0 from "./img-soccer-1.jpg";
import img1 from "./photo-1.png";
import img2 from "./photo-2.png";
import img3 from "./photo-3.png";
import img4 from "./photo-4.png";
import img5 from "./photo-5.png";
import img6 from "./photo-6.png";
import img7 from "./photo-7.png";
import img8 from "./photo-8.png";

import ron from "./ronaldo-2.jpg";
import mes from "./messi.jpg";
import fab from "./fabregas.jpg";
import ram from "./ramos.jpg";
import ney from "./neymar.jpg";

import noPict from "./no-pict.png"

import pp1 from './photo-prof.png'
import pp2 from './photo-prof2.png'
import pp3 from './photo-prof3.png'
import pp4 from './photo-prof1.png'

const Photos = [
  { name: 'img0', file: img0 },
  { name: 'img1', file: img1 },
  { name: 'img2', file: img2 },
  { name: 'img3', file: img3 },
  { name: 'img4', file: img4 },
  { name: 'img5', file: img5 },
  { name: 'img6', file: img6 },
  { name: 'img7', file: img7 },
  { name: 'img8', file: img8 },
  { name: 'ronaldo7', file: ron },
  { name: 'messi10', file: mes },
  { name: 'fabregas4', file: fab },
  { name: 'neymar11', file: ney },
  { name: 'no-pict', file: noPict }
]

export const LandingPagePhoto = [
  { name: 'img1', file: img1 },
  { name: 'img2', file: img2 },
  { name: 'img3', file: img3 },
  { name: 'img4', file: img4 },
  { name: 'img5', file: img5 },
  { name: 'img6', file: img6 },
  { name: 'img7', file: img7 },
  { name: 'img8', file: img8 },
]

export const PhotoCircle = [
  { name: 'pp1', file: pp1 },
  { name: 'pp2', file: pp2 },
  { name: 'pp3', file: pp3 },
  { name: 'pp4', file: pp4 },
]

export default Photos